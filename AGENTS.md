...
- Results use `schema_version: 2`: physical bodies always expose `solutions.geocentric` and add `solutions.topocentric` only for an observer. The topocentric state is minimal local ENU (`x=east`, `y=north`, `z=ellipsoidal up`) with its velocity and light time; do not add redundant angular or distance fields. Mathematical points remain geocentric and have no invented distance or physical state. Do not reintroduce zodiac signs or formatted longitude strings into the API.
