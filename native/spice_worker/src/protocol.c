/*
 * protocol.c — length-prefixed JSON read/write for packet:4 Port protocol.
 */
...
/* Read exactly n bytes from stdin into buf. Returns 0 on success, -1 on EOF/error. */
...
char *read_packet(void) {
  ...
  if (read_exact(hdr, 4) != 0) return NULL;

  uint32_t length =
      ((uint32_t)hdr[0] << 24) | ((uint32_t)hdr[1] << 16) |
      ((uint32_t)hdr[2] << 8)  |  (uint32_t)hdr[3];

  if (length == 0 || length > PROTOCOL_MAX_PAYLOAD) return NULL;
...
int write_packet(const char *json, uint32_t length) {
  ...
  if (fwrite(json, 1, length, stdout) != length) return -1;
  fflush(stdout);
  return 0;
}
